...
void nearbyPointsSearch(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol, int curr_index, std::vector<bool>& processed, std::vector<int>& cluster)
{
	// mark processed
...
int main ()
{
	...
	/*
	int i = 0;
	//Improved insertion in KD tree  
	while(points.size())
	{
		//Improved KD tree
		int evenOrOdd = i % 2; //Even or odd for 2D

		if (evenOrOdd) { // if less, left branch
			sort(points.begin(), points.end() , [](const std::vector<float>& v1,
		...
		tree->insert(median, i);
		points.erase(median_it);
